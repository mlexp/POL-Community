...
class ContentSearch
{
    ...
    public function query(string $term, ?User $user, array $types = ['diary', 'community', 'announcement']): Builder
    {
        ...
        $diaries = $access->diaries($user)->join('users as diary_users', 'diaries.user_id', '=', 'diary_users.id')->where(fn ($q) => $q->whereRaw("diaries.title LIKE ? ESCAPE '!'", [$pattern])->orWhereRaw("diaries.body_html LIKE ? ESCAPE '!'", [$pattern]))->select(['diaries.id', 'diaries.title', 'diaries.excerpt as summary', 'diaries.published_at as date', 'diary_users.display_name as author'])->selectRaw("'diary' as kind");
        ...
        })->select(['community_threads.id', 'community_threads.title', 'community_threads.title as summary', 'community_threads.last_posted_at as date', 'thread_users.display_name as author'])->selectRaw("'community' as kind");
        $announcements = $access->announcements($user)->leftJoin('users as announcement_users', 'announcements.created_by_user_id', '=', 'announcement_users.id')->where(fn ($q) => $q->whereRaw("announcements.title LIKE ? ESCAPE '!'", [$pattern])->orWhereRaw("announcements.summary LIKE ? ESCAPE '!'", [$pattern]))->select(['announcements.id', 'announcements.title', 'announcements.summary', 'announcements.published_at as date', 'announcement_users.display_name as author'])->selectRaw("'announcement' as kind");
