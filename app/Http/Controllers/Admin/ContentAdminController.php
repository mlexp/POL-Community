...
class ContentAdminController extends Controller
{
    public function content(Request $request): View
    {
        ...
        if (! isset($data['type']) || $data['type'] === 'thread') {
            $rows = $rows->concat($apply(DB::table('community_threads')->leftJoin('users', 'community_threads.created_by_user_id', '=', 'users.id')->whereNull('community_threads.deleted_at'), 'community_threads.title', 'users.display_name', 'community_threads')->get(['community_threads.id', DB::raw("'thread' as type"), 'community_threads.title', DB::raw("CASE WHEN community_threads.created_by_user_id IS NULL THEN '非ログインユーザー' ELSE COALESCE(users.display_name, '削除済みユーザー') END as author"), 'community_threads.created_at', 'community_threads.updated_at']));
        }
